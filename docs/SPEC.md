...
- **`user_stats`** — owner anthropometrics, injury history and training
  preferences, 1:1 with `profiles` on the PK: `body_weight_kg, height_cm,
  birth_year, gender, body_type, injuries jsonb` (migration 0020) plus
  `goals jsonb, experience, days_per_week, equipment jsonb,
  preferred_plan_weeks` (migration 0030).
  Edited from the **You** group on `/app/settings` and `/app/you`
  (`UserStatsPanel.tsx`; `goals` is owned by `GoalsEditor.tsx`).
  **A separate table, not columns on `profiles`, for one reason:**
  ...
  Two fields reach a metric: `body_weight_kg` prices unweighted work
  ...
  ranked **below** an observed `hr_max`, never above it.
  Everything else — height, gender, injuries and all five preference columns —
  is read by `buildPlanAiPrompt`, which renders it into the plan-authoring
  prompt (§ below). `Injury.region` is a `RegionKey`, which is what lets the
  prompt name a region the rubric has substitution rules for. `body_type`
  remains the deliberate exception, stored and read by nothing.
  `goals` is a **jsonb array, not an object**: list order is the priority rank.
  `UserStatsInput` is `Partial` because two forms write this one row and neither
  may clobber the other's columns.
...
- **AI prompt** — generated by `buildPlanAiPrompt`, which takes a
  `PlanPromptContext` (`{ stats, today? }`) and defaults to a null profile, so
  it stays synchronous and every existing caller keeps working. It enumerates
  the canonical sections, metrics, units, and block keys read live from
  `app.config.ts`, and adds four things on top:
  - an **ATHLETE PROFILE** block rendered from the caller's `user_stats` row —
    age via `ageFrom`, sex, bodyweight, height, injuries, ranked goals,
    experience, days/week, equipment, preferred length. Goals render by
    **label**, not key, so free-text goals survive and the rubric's wording
    (`skill work`) matches the config key (`skill`). Fields with no value are
    listed under `UNKNOWN` — absence is what the interview asks about.
    `body_type` is never rendered.
  - a **two-phase structure**. Phase 1 is prose: propose a composition, justify
    it, ask for the plan length (bounded by `PLAN_LENGTH`, 6–12 weeks) and
    anything `UNKNOWN`, then wait for confirmation. Phase 2 is the CSV and
    nothing else. This replaced a bare "Produce ONLY the CSV" instruction, which
    made the generator a black box.
  - the **PRESCRIPTION RUBRIC** — `renderRubric()` from `src/lib/planRubric.ts`,
    which imports `docs/PLAN_RUBRIC.md` with `?raw`. The markdown is the single
    source; the module owns only the shape. `docs/PLAN_RUBRIC_PROMPT.md` records
    the prompt that generated it and says to regenerate wholesale, not patch.
  - a **SELF-CHECK** list and a **WORKED EXAMPLE**, where the example is
    `buildPlanCsvTemplate()` itself. The example must be the last thing in the
    prompt: `planTemplate.test.ts` slices it back out at the final header
    occurrence and asserts it parses with zero issues, which is what stops the
    example drifting from the parser. (Appending anything after it breaks that
    test — it already caught exactly this in `sessionTemplate.ts`.)
- **Repair loop** — when the checker reports issues, `PlanUpload` offers **Copy
  fix request**: `buildPlanFixPrompt(issues, csvText)` numbers the exact
  validator strings, restates the header and the legal vocabulary (the original
  prompt has usually scrolled out of the chat), and echoes the rejected CSV —
  omitting the echo for `.xlsx` uploads, where `csvText` is empty.
  `buildSessionFixPrompt` is the session mirror.
...
#### Session authoring surface (AI-friendly)

`/app/plan/upload` carries a `Plan | Sessions` target toggle. In Sessions mode
the same download/copy/upload row produces session-shaped output instead of
plan-shaped output, mirroring the plan authoring surface above but saving N
standalone `sessions` rows (`is_mini=false`, no `source_plan_id`) instead of
one `plans` row:

- **CSV/TSV template** — a rectangular file with the fixed header
  `kind,id,label,section,metric,rounds,planned,notes` and six row kinds
  (`META`, `SESSION`, `GROUP`, `EX`, `SUB`, `VARIANT`) — one file can bundle
  many sessions. Built by `buildSessionCsvTemplate` / `buildSessionTsvTemplate`
  in `src/lib/sessionTemplate.ts`.
- **AI prompt** — generated by `buildSessionAiPrompt`. It enumerates the
  canonical sections and metrics read live from `app.config.ts`, plus the
  `SessionType`/`ScalingLevel`/`GroupKind` unions from `lib/types.ts`.
- **Upload + validation** — `parseSessionTabular` produces `SessionInput[]`
  and a list of compatibility issues; `validateSessions` is the final gate
  before save. The `PlanUpload` UI blocks save while issues remain.
- **Workbook uploads** — `.xlsx` files are read by `parseSessionWorkbook`
  (dynamic `exceljs` import), same header-locating strategy as
  `parsePlanWorkbook`. No markdown source exists for sessions — CSV/TSV/XLSX
  only.
- **Full frame in v1** — a session's `frame` can carry metadata, `groups`
  (single/superset/circuit, with rounds and rest), and Rx/Intermediate/
  Beginner scaling `variants`, matching the `SessionFrame` contract exactly.

When the session/frame structure, sections, metrics, or the `SessionType` /
`ScalingLevel` / `GroupKind` unions change, update `sessionTemplate.ts`
(sample rows + prompt) and its tests in the same change. This is a hard rule
in `CLAUDE.md`.
