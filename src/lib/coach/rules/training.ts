...
export function rpeCalibration(s: TrainingSignals, periodKey: string): Finding | null {
  ...
  return {
    ...
    drift:
      gap != null && gap < -0.5
        ? shareOf(unratedSessions, ratedSessions + unratedSessions)
        : shareOf(defaultShare, 1),
...
export function consecutiveDays(
  ...
  return {
    ...
    drift: shareOf(s.longestConsecutiveDays - run + 1, 5),
    confidence: 0.35,
    sufficiency: 'ok',
